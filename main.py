...
def convert_pdf_to_html(path: Text, definitions: Dict) -> Text:
    ...
    for key, value in definitions.items():
        output_html = re.sub(key, f'<span style="cursor:pointer;color:blue;text-decoration:underline;" id="{"".join(key.lower().split())}">{key}</span>', output_html, flags=re.IGNORECASE)
    return(output_html)
